    ...
    <div class="">
      ...
      <v-window v-model="activeTab">
        <v-window-item value="water">
          ...
          <v-table v-if="filteredWaterFiles.length > 0" class="mt-4 border-sm rounded-lg table-u">
            <thead>
              <tr>
                <th>ລະຫັດຟາຍ</th>
                <th>ຊື່ຟາຍອັບໂຫລດ</th>
            ...
            <tbody>
              <tr v-for="(file, index) in filteredWaterFiles" :key="index">
                ...
                <td>
                  <v-chip :color="file.status === 'Approved' ? 'green' : 'orange'">
                    {{ file.status }}
                  </v-chip>
                </td>
                <td>
                  <v-btn color="red" icon @click="deleteWaterFile(file.id)" class="mr-2">
                  ...
                  <v-btn
                    ...
                    :disabled="file.status === 'Approved' || isWaterProcessing[file.id]"
                  >
                    <v-icon>mdi-check</v-icon>
                  </v-btn>
                </td>
                <td>
                  <div v-if="waterFileProgress[file.id] !== undefined" class="progress-container">
                    <v-progress-linear
        ...
        <v-window-item value="electric">
          ...
          <v-table v-if="filteredElectricFiles.length > 0" class="mt-4 border-sm rounded-lg table-u">
            <thead>
              <tr>
                <th>ລະຫັດຟາຍ</th>
                <th>ຊື່ຟາຍອັບໂຫລດ</th>
            ...
            <tbody>
              <tr v-for="(file, index) in filteredElectricFiles" :key="index">
                ...
                <td>
                  <v-btn color="red" icon @click="deleteElectricFile(file.id)" class="mr-2">
                    <v-icon>mdi-delete</v-icon>
                  </v-btn>
                  <v-btn
                    color="green"
                    ...
                    :disabled="file.status === 'Approved' || isElectricProcessing[file.id]"
                  >
                    <v-icon>mdi-check</v-icon>
                  </v-btn>
                </td>
                <td>
                  <div v-if="electricFileProgress[file.id] !== undefined" class="progress-container">
                    <v-progress-linear
...
const years = [null, ...Array.from({ length: 10 }, (_, i) => new Date().getFullYear() - i)];
...
const fetchWaterFiles = async () => {
  try {
    ...
    console.error('Error fetching water files:', error.response?.data || error.message);
  }
};
...
const fetchElectricFiles = async () => {
  try {
    ...
    console.error('Error fetching electric files:', error.response?.data || error.message);
  }
};
...
const handleWaterFileUpload = async () => {
  if (!waterUploadedFiles.value || waterUploadedFiles.value.length === 0) return;

  const invalidFiles = waterUploadedFiles.value.filter(file => 
    !file.name.toLowerCase().startsWith('water-bill')
  );
  if (invalidFiles.length > 0) {
    waterUploadError.value = 'Only files starting with "water-bill" are allowed in the Water tab.';
  ...
  waterUploadedFiles.value.forEach(file => {
    ...
    if (parsed) {
      ...
      console.log(`Set filters from ${file.name}: Month=${parsed.month}, Year=${parsed.year}`);
    }
  });
  ...
  try {
    ...
    waterUploadError.value = error.response?.data?.error || 'Water upload failed';
    console.error('Error uploading water file:', error.response?.data || error.message);
  }
};

// Handle Electric File Upload with Filtering
const handleElectricFileUpload = async () => {
  if (!electricUploadedFiles.value || electricUploadedFiles.value.length === 0) return;

  const invalidFiles = electricUploadedFiles.value.filter(file => 
    !file.name.toLowerCase().startsWith('electric-bill')
  );
  if (invalidFiles.length > 0) {
    electricUploadError.value = 'Only files starting with "electric-bill" are allowed in the Electric tab.';
  ...
  try {
    ...
    electricUploadError.value = error.response?.data?.error || 'Electric upload failed';
    console.error('Error uploading electric file:', error.response?.data || error.message);
  }
};
...
const confirmWaterFile = async (fileId) => {
  try {
    ...
    const patchResponse = await axios.patch(`${WATER_API_URL}${fileId}/`, { status: 'Approved' });
    console.log(`Water File ${fileId} approved with response:`, patchResponse.data);
    pollWaterProgress(fileId);
  } catch (error) {
    console.error('Error confirming water file:', error.response?.data || error.message);
...
const confirmElectricFile = async (fileId) => {
  try {
    ...
    const patchResponse = await axios.patch(`${ELECTRIC_API_URL}${fileId}/`, { status: 'Approved' });
    console.log(`Electric File ${fileId} approved with response:`, patchResponse.data);
    pollElectricProgress(fileId);
  } catch (error) {
    console.error('Error confirming electric file:', error.response?.data || error.message);
...
const pollWaterProgress = async (fileId) => {
  ...
  const interval = setInterval(async () => {
    ...
    try {
      ...
      console.log(`Water Progress for ${fileId}: ${progress}% of ${total} items`);
      ...
      console.error('Water Polling error:', error.response?.data || error.message);
      clearInterval(interval);
      delete waterFileProgress.value[fileId];
      isWaterProcessing.value[fileId] = false;
    }
...
const pollElectricProgress = async (fileId) => {
  ...
  const interval = setInterval(async () => {
    ...
    try {
      const response = await axios.get(`${ELECTRIC_API_URL}${fileId}/progress/`);
      ...
      console.log(`Electric Progress for ${fileId}: ${progress}% of ${total} items`);
      if (completed || progress >= 100) {
      ...
      console.error('Electric Polling error:', error.response?.data || error.message);
      clearInterval(interval);
      delete electricFileProgress.value[fileId];
      isElectricProcessing.value[fileId] = false;
    }
...
const deleteWaterFile = async (fileId) => {
  try {
    ...
    console.error('Error deleting water file:', error.response?.data || error.message);
  }
};
...
const deleteElectricFile = async (fileId) => {
  try {
    ...
    console.error('Error deleting electric file:', error.response?.data || error.message);
  }
};
...
const filteredWaterFiles = computed(() => {
  return waterFiles.value.filter(file => {
    const matchesSearch =
      searchQuery.value === '' || file.file_name.toLowerCase().includes(searchQuery.value.toLowerCase());
    const matchesStatus =
      selectedStatus.value === '' || file.status === selectedStatus.value;
    const matchesYear =
      selectedYear.value === null || new Date(file.insertDate).getFullYear() === selectedYear.value;
    const fileDate = getFileMonthYear(file.file_name);
    const matchesMonth =
      selectedMonth.value === '' || (fileDate && fileDate.month === months.indexOf(selectedMonth.value));
