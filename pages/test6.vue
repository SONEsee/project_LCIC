  ...
  <div>    <h2 class="head-user">ລາຍງານການຄົ້ນຫາ</h2>
    ...
    <div class="table-summary border-md">
      <v-row class="justify-space-between align-center" style="height: 60px;">
        ...
        <v-col class="text-center">
          <p class="text-highlight">ຈໍານວນການຄົ້ນຫາທັງໝົດ : {{  }} ກີບ</p>
        </v-col>
        <v-col class="text-center">
...
export default {
  ...
  try {
    ...
    if (data && data.searchlog) {
      this.searchlog = data.searchlog;
    } else {
      console.warn('No charge data found in response:', data);
      this.searchlog = []; // Set to empty array if data.charge is missing
    }
  } catch (error) {
