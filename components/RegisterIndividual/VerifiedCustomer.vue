        ...
        <div class="modal-body-split">
          <div class="info-column">
            <div class="info-block">
              ...
              <!-- Customer ID - Editable in Modal -->
              <div class="info-row">
                <span class="label">ລະຫັດລູກຄ້າ:</span>
                <div v-if="editingCustomerId === selectedDetail.ind_sys_id" class="edit-inline-modal">
                  <input 
                    v-model="newCustomerId"
                    type="text"
                    placeholder="ລະຫັດ"
                    class="edit-input"
                    @keyup.enter="saveCustomerId(selectedDetail)"
                    @keyup.esc="cancelEdit"
                  />
                  <button @click="saveCustomerId(selectedDetail)" class="btn-save">✓</button>
                  <button @click="cancelEdit" class="btn-cancel">✕</button>
                </div>
                <div v-else>
                  <code>{{ selectedDetail.customer_id || '-' }}</code>
                  <button 
                    v-if="canEditCustomer(selectedDetail) && selectedDetail.confirmed && !selectedDetail.customer_id"
                    @click="startEditCustomerId(selectedDetail)"
              ...
              <!-- Segment - Editable in Modal -->
              <div class="info-row">
                <span class="label">ກຸ່ມ:</span>
                <div v-if="editingSegment === selectedDetail.ind_sys_id" class="edit-inline-modal">
                  <select 
                    v-model="newSegment"
                    class="edit-select"
                    @keyup.enter="saveSegment(selectedDetail)"
                    @keyup.esc="cancelEdit"
                  >
                    <option value="A1">A1</option>
                    <option value="A3">A3</option>
                  </select>
                  <button @click="saveSegment(selectedDetail)" class="btn-save">✓</button>
                  <button @click="cancelEdit" class="btn-cancel">✕</button>
                </div>
                <div v-else>
                  <span class="segment-badge">{{ selectedDetail.segment || '-' }}</span>
                  <button 
                    v-if="canEditCustomer(selectedDetail)"
                    @click="startEditSegment(selectedDetail)"
                    class="btn-edit-modal"
                    title="ແກ້ໄຂ"
                  >
                    ✏️ ແກ້ໄຂ
...
const saveCustomerId = async (customer: any) => {
  ...
  try {
    ...
    if (selectedDetail.value) {
      const updated = customers.value.find(c => c.ind_sys_id === selectedDetail.value.ind_sys_id);
