      ...
      <v-card-text>
        <v-window v-model="tab">
          <v-window-item value="one">
            <h3>ຂໍ້ມູນທີ່ອັບໂຫຼດທັງໝົດຂອງ: <b>ໂຄງການ.</b> </h3>
            <v-table>
              <thead>
          ...
          <v-window-item value="two">
            ...
            <v-window v-model="subTab">
              <v-window-item value="two-one">
                <h3>ຂໍ້ມູນ <b> ໂຄງການ.</b> ທີ່ບໍ່ມີ ແລະ ຜິດ Enterprise Code </h3>
              ...
              <v-window-item value="two-two">
                <v-btn @click="exportToJson" class="bg-red"
                  ...
                  JSON</v-btn
                >
                <h3 class="mt-5">ຂໍ້ມູນ <b> ໂຄງການ.</b>ບໍຖືກ ແລະບໍ່ມີ LcicID </h3>:
                <v-col cols="12">
                  <v-row>
              ...
              <v-window-item value="two-three">
                <h3>
                  ຂໍ້ມູນ <b> ໂຄງການ.</b> ທີ່ມີ LCICID ແລະ com_enterprise_code_error ຜິດ ຫຼື ບໍ່ມີ
                </h3>
              ...
              <v-window-item value="two-five">
                <h3>
                  ຂໍ້ມູນ <b> ໂຄງການ.</b>ທີ່ມີ bnk_code, branch_id, customer_id, loan_id ແຕ່ມີ
          ...
          <v-window-item value="three">
              ...
              <v-data-table :items="prj" :header="headers">
                <template v-slot:header>
                  <tr style="color: black; background-color: blue"></tr>
                </template>
                <template v-slot:item="{ item }">
    ...
    setup() {
      ...
      onMounted(() => {
        ...
        if (queryData) {
          ...
          colrealestates.value = parsedData.col_project_prj || [];
      ...
      const combinedData = computed(() => {
        const filteredCerror = (cerror.value || []).filter(
          (item) =>item.col_type === "C2.4" ||item.col_type === "c2.4"
        );
    ...
    const prj = computed(() => {
        ...
        return cerror.value
          .filter(
            (item) =>
              (item.collateral_status === "31" ||
                item.collateral_status === "10") &&
             item.col_type === "C2.4" ||item.col_type === "c2.4"
          )
          .map((item) => ({
            id: item?.id || null,
            lcicid: item?.lcicID || null,
      ...
      const lcicerror = computed(() => {
        return cerror.value
          .filter(
            (item) =>
             ( item.collateral_status === "13" ||item.collateral_status === "01") &&item.col_type === "C2.4" ||item.col_type === "c2.4"
          )
          .map((item) => ({
            id: item?.id || null,
            lcicid: item?.lcicID || null,
